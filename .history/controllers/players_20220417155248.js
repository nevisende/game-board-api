...
async function getHighHundredRanked(req, res) { 
  try {
    const hundredPlayers2 = await Player.find().limit(100);
    
    const hundredPlayers = hundredPlayers2.map((player) => {
      const newObject = {};
      delete Object.assign(newObject, player, {"playera": player["_id"]})["_id"]
      return newObject
    })
