...
async function getHighHundredRanked(req, res) { 
  try {
    const hundredPlayers = (index.redisClient.dbSize() > 100 )  ? await  index.redisClient.get("*") : await Player.find({ isThisWeekActive: true });
...
async function createFakePlayers(req, res) {
  ...
  const isThisWeekActive = req.body.is_this_week_active
  let playersList = [];
  let randomString = ""
